...
class BERT4Rec_encoder(ContextEncoder):

    def __init__(self, config):
        super().__init__(config)
        self.network_param_init(config)
        print('###################BERT4Rec ENCODER YES###############')
    ...
    def seq_modelling(self, hist_item_ids, user_embed=None, rectify=True):
        ...
        bert_context = self.seq_module(hist_item_ids, outer_embed=self.item_emb.weight, unidirectional=False)
        # [bs, hidden_size]
        if rectify:
