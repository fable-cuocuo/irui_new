...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config, use_outer_embed=False):
        ...
        # 添加可學習的二進制掩碼
        self.mask = nn.Parameter(torch.randn(config.max_position_embeddings) * 0.01)  # 初始化可学习的掩码
        self.sigmoid = nn.Sigmoid()  # 用于将掩码值转换到0到1之间
    ...
    def forward(self, input_ids, token_type_ids=None, outer_emb=None):
        ...
        mask_values = self.sigmoid(self.mask).unsqueeze(0).unsqueeze(2)  # 将掩码扩展到与嵌入相同的维度
        embeddings = embeddings * mask_values  # 应用掩码
...
class BertModel(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=False, outer_embed=None):
