...
class SeqRec(nn.Module):
    ...
    def seqs_correction(self, input_seqs, masks, targets, neg_targets, changed_index, candidate_items):
        ...
        while i < len(changed_target):
            ...
            neg_targets = all_neg_targets[seqs_index].unsqueeze(0)
            # [5,4,50]
            ...
            rec_output = self.recommender_forward(one_input, one_mask) # rec_output
            # 新target的rank
            changedTarget = candidate_items[seqs_index][t_c_index] #new target index
            changedTarget = torch.cat([changedTarget, neg_targets], dim=1)
            target_emb = item_embed[changedTarget] # new target emb
            
            # rank: rec_output with new target

            scores = torch.mul(rec_output.unsqueeze(1), target_emb).sum(dim=2, keepdim=False)
            scores[changedTarget == 0] == -100000
            pos_score = scores[:, 0: 1]
            neg_scores = scores[:, 1: -1]
            new_rank = (neg_scores > pos_score).long().sum(dim=1, keepdim=False)
            
            # rank: rec_output with old target
            org_target = targets[seqs_index].unsqueeze(0).unsqueeze(0)
            org_target = torch.cat([org_target, neg_targets], dim=1)
            org_target_emb = item_embed[org_target]
            org_scores = torch.mul(rec_output.unsqueeze(1), org_target_emb).sum(dim=2, keepdim=False)
            org_scores[org_target == 0] == -100000
            org_pos_score = org_scores[:, 0: 1]
            org_neg_scores = org_scores[:, 1: -1]
            org_rank = (org_neg_scores > org_pos_score).long().sum(dim=1, keepdim=False)

            rise = new_rank - org_rank
            if rise <= 0 and candidate_items[seqs_index][t_c_index] != 0:
        ...
        while j < len(hist_changed):
            ...
            # org seqs
            org_seqs = modified_seqs[instance_index]
            one_mask = masks[instance_index].unsqueeze(0)
            org_rec_output = self.recommender_forward(org_seqs.unsqueeze(0), one_mask)

            # modify org seqs
            new_seqs =  modified_seqs[instance_index]
            new_seqs[modified_item_index] = candidate_items[instance_index][item_c_index]
            new_seqs = new_seqs.unsqueeze(0)
            new_rec_output = self.recommender_forward(new_seqs, one_mask)

            # ranks new seqs with target
            ...
            new_scores = torch.mul(new_rec_output.unsqueeze(1), all_targets_embs).sum(dim=2, keepdim=False)
            new_scores[all_targets == 0] == -100000
            new_pos_score = new_scores[:, 0: 1]
            new_neg_scores = new_scores[:, 1: -1]
            new_rank = (new_neg_scores > new_pos_score).long().sum(dim=1, keepdim=False)

            # ranks old seqs with target
            scores = torch.mul(org_rec_output.unsqueeze(1), all_targets_embs).sum(dim=2, keepdim=False)
            scores[all_targets == 0] = -100000
            pos_score = scores[:, 0: 1]
            neg_scores = scores[:, 1: -1]
            org_rank = (neg_scores > pos_score).long().sum(dim=1, keepdim=False)
            

            rise = new_rank - org_rank

            if rise <= 0 and  candidate_items[instance_index][item_c_index] != 0:
