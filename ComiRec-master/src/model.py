...
def dense(inputs, units, activation=None, use_bias=True, name=None, bias_initializer=tf.zeros_initializer()):
    """TF1-style dense layer without tf.layers (Keras 3 incompatible)."""
    if bias_initializer is None:
        use_bias = False
    if name is not None:
        scope_ctx = tf.variable_scope(name)
    else:
        scope_ctx = tf.variable_scope(None, default_name='dense')
    with scope_ctx:
        in_dim = inputs.get_shape().as_list()[-1]
        if in_dim is None:
            in_dim = tf.shape(inputs)[-1]
        kernel = tf.get_variable(
            'kernel', shape=[in_dim, units], dtype=inputs.dtype,
            initializer=tf.glorot_uniform_initializer())
        output = tf.tensordot(inputs, kernel, axes=[[-1], [0]])
        if use_bias:
            bias = tf.get_variable(
                'bias', shape=[units], dtype=inputs.dtype, initializer=bias_initializer)
            output = output + bias
...
class Model_DNN(Model):
    def __init__(self, n_mid, embedding_dim, hidden_size, batch_size, seq_len=256):
        ...
        self.user_eb = dense(self.item_his_eb_mean, hidden_size, activation=None)
...
class CapsuleNetwork(object):
    ...
    def __call__(self, item_his_emb, item_eb, mask):
        return self.call(item_his_emb, item_eb, mask)

    def call(self, item_his_emb, item_eb, mask):
        with tf.variable_scope('bilinear'):
            if self.bilinear_type == 0:
                item_emb_hat = dense(item_his_emb, self.dim, activation=None, bias_initializer=None)
                item_emb_hat = tf.tile(item_emb_hat, [1, 1, self.num_interest])
            elif self.bilinear_type == 1:
                item_emb_hat = dense(item_his_emb, self.dim * self.num_interest, activation=None, bias_initializer=None)
            else:
                w = tf.get_variable(
        ...
        if self.relu_layer:
            interest_capsule = dense(interest_capsule, self.dim, activation=tf.nn.relu, name='proj')
...
class Model_ComiRec_SA(Model):
    def __init__(self, n_mid, embedding_dim, hidden_size, batch_size, num_interest, seq_len=256, add_pos=True):
        ...
        with tf.variable_scope("self_atten", reuse=tf.AUTO_REUSE) as scope:
            item_hidden = dense(item_list_add_pos, hidden_size * 4, activation=tf.nn.tanh)
            item_att_w  = dense(item_hidden, num_heads, activation=None)
